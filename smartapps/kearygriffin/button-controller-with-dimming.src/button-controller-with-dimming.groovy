...
definition(
    ...
    author: "kearygriffin",
    description: "Control devices with buttons w/dimming",
...
def selectButton() {
	dynamicPage(name: "selectButton", title: "First, select your button device", nextPage: "configureButton1", uninstall: configured()) {
		section {
            ...
            input(
                name		: "dimLevel"
                ,title		: "Select dim level to set when turning on"
                ,multiple	: false
                ,required	: false
                ,type		: "enum"
...
def toggle(devices) {
	...
	if (devices*.currentValue('switch').contains('on')) {
		...
		devices.on()
        if (dimLevel && dimLevel.length())
           devices.setLevel(dimLevel.toInteger());
	}
