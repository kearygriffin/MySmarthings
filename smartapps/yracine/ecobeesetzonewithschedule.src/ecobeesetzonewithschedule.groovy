...
def get_APP_VERSION() {return "8.1.2"}
				...
				if (isRoomOccupied(motionSensor, indiceRoom)) {
					...
					if (roomTstat) {
						...
						traceEvent(settings.logFilter,"setAllRoomTstatsSettings>schedule ${scheduleName},room ${roomName} not occupied, not sending setpoints", settings.detailedNotif,
							get_LOG_INFO()) 
						if ((settings.turnOffRoomTstatsFlag) && (currentMode != 'off')) {
							traceEvent(settings.logFilter,"schedule ${scheduleName},$roomTstat to be turned off as turnOffRoomTstatsFlag=true",
								settings.detailedNotif)            
							roomTstat.off()                           
						}
					}
...
private void turn_off_room_tstats_outside_zones(indiceSchedule) {   
	...
	for (int indiceRoom =1; ((indiceRoom <= settings.roomsCount) && (indiceRoom <= get_MAX_ROOMS())); indiceRoom++) {
		...
		if (!found_room_in_zone) { 		        
			try {        
				roomTstat.off()   // send the off command to the room tstat as it's outside the zones for the schedule     
				traceEvent(settings.logFilter,"schedule $scheduleName, turning off $roomTstat as $roomName is outside the zone ($zones)",
					settings.detailedNotif, get_LOG_INFO(), settings.detailedNotif)
			} catch (e) {
				traceEvent(settings.logFilter,"exception $e, schedule $scheduleName, not able to turn off $roomTstat, as $roomName is outside the zone ($zones)",
					settings.detailedNotif, get_LOG_WARN(), settings.detailedNotif)
			}
		}
	} /* end for each room */
...
//		def indoorTemps = getAllTempsForAverage(indiceZone,refreshSensorsFlag) // Commented out to avoid any "offline" issues with some sensors following some ST platform changes
	...
	for (zone in zones) {
		...
		if (mode.contains('heat')) { 
			desiredTemp = desiredTemp + ((desiredHeatDelta)?desiredHeatDelta.toFloat():0)  
			...
			desiredTemp = desiredTemp + ((desiredCoolDelta)?desiredCoolDelta.toFloat():0) 
