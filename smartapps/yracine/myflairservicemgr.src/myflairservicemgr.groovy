/**
 *  MyFlairServiceMgr (Service Manager)
...
def about() {
 	...
 	dynamicPage(name: "about", install: false, uninstall: true) {
 		section("") {	
			paragraph "${get_APP_NAME()}, the smartapp that connects your Flair devices to SmartThings via cloud-to-cloud integration"
			paragraph "Version ${get_APP_VERSION()}" 
			paragraph "If you like this smartapp, please support the developer via PayPal and click on the Paypal link below " 
				href url:"https://www.paypal.me/ecomatiqhomes",
					title:"Paypal donation..."
			paragraph "Copyright©2016 Yves Racine"
				href url:"http://github.com/yracine/device-type.myflair", style:"embedded", required:false, title:"More information...", 
					description: "http://github.com/yracine/flairdevices"
		}
	}        
}
	...
	if (!atomicState?.oauthTokenProvided) {
		...
		return dynamicPage(name: "auth", title: "Login", nextPage:null, uninstall:uninstallAllowed, submitOnChange: true) {
			section(){
				...
				href url:redirectUrl, style:"embedded", required:true, title:"Flair Connect>", description:description
			}
		}
		...
		return dynamicPage(name: "auth", title: "Log In", nextPage:"structureList", uninstall:uninstallAllowed,submitOnChange: true) {
			section(){
				...
				href url:redirectUrl, style:"embedded", state:"complete", title:"Flair", description:description
			}
		}
...
def structureList() {
	...
	dynamicPage(name: "structureList", title: "Select Your Location to be exposed to SmartThings.", uninstall: true) {
		section(""){
...
def puckDeviceList() {
	...
	traceEvent(settings.logFilter,"puckDeviceList>device list: $pucksList", detailedNotif)

	def p = dynamicPage(name: "puckDeviceList", title: "Select Your Puck Device(s) to be exposed to SmartThings.", uninstall: true) {
		section(""){
			paragraph image: "${getCustomImagePath()}pressure.png","Tap below to see the list of Flair Puck Devices available in your Flair account."
...
def flairTstatList() {
	...
	def p = dynamicPage(name: "flairTstatList", title: "Select Your Flair Thermostats to be exposed to SmartThings.", uninstall: true) {
		section(""){
        
			paragraph image: "${getCustomImagePath()}home1.png", "Tap below to see the list of Flair Tstats available in your Flair account. The thermostats will be exposed to SmartThings in read-only mode."
...
def flairHvacList() {
	...
	traceEvent(settings.logFilter,"flairHvacList>device list: $hvacs", detailedNotif)

	def p = dynamicPage(name: "flairHvacList", title: "Select Your Flair HVAC unit(s) to be exposed to SmartThings.", uninstall: true) {
		section(""){
			paragraph image: "${getCustomImagePath()}split.png","Tap below to see the list of Flair HVAC units (windows, splits, heaters, etc) available in your Flair account. "
...
def flairVentList() {
	...
	traceEvent(settings.logFilter,"flairVentList>device list: $vents", detailedNotif)

	def p = dynamicPage(name: "flairVentList", title: "Select Your Flair Vents to be exposed to SmartThings.", uninstall: true) {
		section(""){
			paragraph image: "${getCustomImagePath()}ventopen.png","Tap below to see the list of Flair Vents available in your Flair account. "
...
//						log.debug "getObject>objects[dni]= ${objects[dni]}"
		...
		traceEvent(msg, true, get_LOG_ERROR())   
...
private void create_child_pucks() {
	...
	def devices = pucks.collect { dni ->
		...
		if(!d) {
			...
			d = addChildDevice(getChildNamespace(), getPuckChildName(), dni, null,
				[label: "${labelName}"]) 
			d.initialSetup( getSmartThingsClientId() , getSmartThingsClientSecretId(), atomicState.access_token,atomicState.refresh_token,atomicState.token_type,atomicState.authexptime, 
...
def initialize() {
	...
	delete_child_devices()	
	runIn((60*1), "create_child_tstats") // runIn is used with a delay to instantiate the different objects
	runIn((60*1), "create_child_pucks")
	runIn((60*1), "create_child_vents")
	runIn((60*1), "create_child_hvacs")
